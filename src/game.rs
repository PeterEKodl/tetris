...
impl Game {
    ...
    pub fn rotate(&mut self, rotation: actions::Rotation) {
        let mut brick_rotated = self.brick;
    ...
    fn move_brick_down(&mut self) {
        if !check_overlap(&self.brick, &self.grid, self.brick_x, self.brick_y + 1) {
            ...
            self.move_interval = (Self::INITIAL_MOVE_INTERVAL
                - Duration::from_millis((self.get_level() * 40) as u64))
...
impl render::Render for Game {
    fn render(&self, rustbox: &RustBox) {
        ...
        render_brick(
            (corner_x as isize + 1 + self.brick_x) as usize,
            (corner_y as isize + 1 + self.brick_y) as usize,
            &self.brick,
            rustbox,
        );
    ...
    for y in 0..brick.len {
        for x in 0..brick.len {
            if brick.get(x, y) == 1 {
                ...
                if (0..(grid.width as isize)).contains(&grid_x)
                    && (0..(grid.height as isize)).contains(&grid_y)
                {
