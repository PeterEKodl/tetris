...
fn main() {
    let rustbox = RustBox::init(InitOptions::default())
        .expect("Failed to initalize terminal.");
    ...
    loop
    {
        ...
        match rustbox.peek_event(Duration::new(0, 1_000_000), false).unwrap()
        {
            Event::KeyEvent(key) =>
            {
                match key
                {
                    Key::Esc => if pause_and_check_quit(&rustbox, &game) {break;} else {now = Instant::now();continue;},
                    Key::Char('q') => break,
...
{
    game.render(&rustbox);   
    rustbox.print(0, rustbox.height() - 1, RB_REVERSE, Color::White, Color::Default, CONTROLS);
    rustbox.print(0, 0, RB_NORMAL, Color::White, Color::Default, &format!("FPS: {}", fps as u32));
}
...
fn pause_and_check_quit(rustbox: &RustBox, game:&game::Game) -> bool
{
    loop
    {
        ...
        rustbox.print((rustbox.width()-5)/2, 0, RB_BOLD, Color::White, Color::Red, "PAUSE");
        rustbox.present();
